  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: mobile ? "flex-start" : "center", marginBottom: 16, flexDirection: mobile ? "column" : "row", gap: mobile ? 8 : 0 }}>
        <h2 style={{ margin: 0, color: "#003366", fontSize: mobile ? 20 : 24 }}>Work Queue</h2>
      ...
      {groups.length === 0 ? (
        <div style={{ background: "#fff", borderRadius: 10, padding: mobile ? 24 : 40, textAlign: "center", color: "#888", boxShadow: "0 1px 4px rgba(0,0,0,0.1)" }}>
          No items in work queue.<br />
          <span style={{ fontSize: mobile ? 12 : 13 }}>Open an inquiry and click <b>+ Work Queue</b> on any product.</span>
        </div>
      ) : (
        groups.map(group => (
          <div key={group.InquiryID} style={{ background: "#fff", borderRadius: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.1)", marginBottom: 20, overflow: "visible" }}>
            ...
            <div style={{ background: "#003366", color: "#fff", padding: mobile ? "12px 12px" : "12px 20px", display: "flex", justifyContent: "space-between", alignItems: mobile ? "flex-start" : "center", flexDirection: mobile ? "column" : "row", gap: mobile ? 10 : 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: mobile ? 8 : 14, flexWrap: "wrap", flex: 1 }}>
                <button onClick={() => onOpenInquiry(group.InquiryID)}
                  style={{ background: "rgba(255,255,255,0.15)", border: "none", color: "#fff", cursor: "pointer", fontSize: 11, fontWeight: 700, borderRadius: 5, padding: "3px 10px", whiteSpace: "nowrap" }}>
                  Inquiry #{group.InquiryID}
                </button>
                <div>
                  <div style={{ fontSize: mobile ? 13 : 14, fontWeight: 600 }}>{group.CustomerName}</div>
                  {mobile && group.CustomerCompany && <div style={{ fontSize: 12, opacity: 0.8 }}>{group.CustomerCompany}</div>}
                  {!mobile && group.CustomerCompany && <span style={{ fontSize: 13, opacity: 0.8 }}>· {group.CustomerCompany}</span>}
                </div>
                <span style={{ fontSize: 11, opacity: 0.7, whiteSpace: "nowrap" }}>{group.items.length} product{group.items.length !== 1 ? "s" : ""}</span>
              </div>
              <button onClick={() => openResponseModal(group).catch(e => console.error("Error opening response modal:", e))}
                style={{ background: "#fff", color: "#003366", border: "none", borderRadius: 6, padding: mobile ? "8px 12px" : "6px 16px", cursor: "pointer", fontSize: 12, fontWeight: 700, width: mobile ? "100%" : "auto" }}>
                ✉ Send Response
              </button>
            </div>
      ...
      {responseModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: mobile ? "flex-end" : "center", justifyContent: "center", zIndex: 1000 }}
          ...
          <div style={{ background: "#fff", borderRadius: mobile ? "16px 16px 0 0" : 12, padding: mobile ? 16 : 28, width: mobile ? "100%" : 580, maxHeight: mobile ? "85vh" : "auto", overflowY: mobile ? "auto" : "visible", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}
            onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
              <h3 style={{ margin: 0, color: "#003366", fontSize: mobile ? 16 : 18 }}>Send Response — Inquiry #{responseModal.InquiryID}</h3>
              <button onClick={() => setResponseModal(null)} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#888" }}>✕</button>
            </div>
            <p style={{ margin: "0 0 12px", fontSize: mobile ? 12 : 13, color: "#666" }}>
              Sending will update the inquiry to <b>Quoted</b> and remove all its products from the work queue.
            </p>
            <textarea value={responseText} onChange={e => setResponseText(e.target.value)} rows={mobile ? 8 : 12}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14, flexDirection: mobile ? "column" : "row" }}>
              <button onClick={handleSendAndClose} disabled={sending || !responseText.trim()}
                style={{ flex: 1, background: sending || !responseText.trim() ? "#ccc" : "#003366", color: "#fff", border: "none", borderRadius: 6, padding: "10px", cursor: sending || !responseText.trim() ? "not-allowed" : "pointer", fontWeight: 600, fontSize: 13 }}>
