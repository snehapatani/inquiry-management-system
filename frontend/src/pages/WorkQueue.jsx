...
import {
  getWorkItems, getQuotes, matchVendors, createQuote, setBestQuote, createVendor, toggleSelectItem, getVendors, sendResponse, updateInquiryStatus,
} from "../api";
...
export default function WorkQueue({ onOpenInquiry, user = null }) {
  ...
  // ── send response & close ────────────────────────────────────
  function openResponseModal(group) {
    ...
    for (const item of group.items) {
      ...
      if (best) {
        ...
        lines.push(`  Price: ${best.Currency} ${best.QuotedPrice}${best.PriceUnit ? " / " + best.PriceUnit : ""}  |  Lead Time: ${best.LeadTimeDays || "—"} days`);
        lines.push("");
      } else {
  ...
  async function handleSendAndClose() {
    ...
    // Check if all products in the inquiry have best prices quoted
    const allItemsQuoted = responseModal.items.every(item => {
      const itemQuotes = quotes[item.ItemID] || [];
      return itemQuotes.some(q => q.IsBestPrice);
    });

    const newStatus = allItemsQuoted ? "Closed" : "Quoted";
  ...
  return (
    <div>
      ...
      {groups.length === 0 ? (
        ...
        groups.map(group => (
          <div key={group.InquiryID} style={{ background: "#fff", borderRadius: 10, boxShadow: "0 1px 4px rgba(0,0,0,0.1)", marginBottom: 20, overflow: "hidden" }}>
            ...
            <div style={{ background: "#003366", color: "#fff", padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => openResponseModal(group)}
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: 28, width: 580, boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              <button onClick={handleSendAndClose} disabled={sending || !responseText.trim()}
                ...
                {sending ? "Saving…" : "Log Response & Close"}
              </button>
